...
import { api, updateLauncherAndWait } from "./api";
...
export function App({ authenticated = true }: { authenticated?: boolean }) {
  ...
  function showError(value: unknown) { setError(value instanceof Error ? value.message : String(value)); }
  ...
  return <div className="app-shell">
    ...
    {error && <div className="banner error"><span>{error}</span><button onClick={() => setError("")}>Dismiss</button></div>}
    <main>
      <section>
      ...
      {!installed ? <>
        ...
        {step === 2 && <Panel title="Configure" description="Choose the stable application and stage names. Region is fixed by the Quick Create link."><form onSubmit={(event) => { event.preventDefault(); void saveStructured(3); }}><div className="grid two"><Field label="Application name"><input value={configuration.appName} onChange={(event) => setConfiguration({ ...configuration, appName: event.target.value })} /></Field><Field label="Production stage"><input value={configuration.productionStage} onChange={(event) => setConfiguration({ ...configuration, productionStage: event.target.value })} /></Field></div><DomainFields configuration={configuration} setConfiguration={setConfiguration} /><Advanced toml={toml} setToml={setToml} onSave={saveToml} /><div className="actions"><button className="primary" type="submit">Save and continue</button></div></form></Panel>}
        {step === 3 && <Panel title="Integrations and administrator" description="Credentials are encrypted in operation-scoped SSM parameters and deleted after use."><IntegrationFields configuration={configuration} setConfiguration={setConfiguration} secrets={secrets} setSecrets={setSecrets} /><div className="divider" /><div className="grid two"><Field label="Administrator email"><input type="email" value={admin.email} onChange={(event) => setAdmin({ ...admin, email: event.target.value })} /></Field><Field label="Display name"><input value={admin.displayName ?? ""} onChange={(event) => setAdmin({ ...admin, displayName: event.target.value })} /></Field><Field label={`Administrator password (${MIN_ADMIN_PASSWORD_LENGTH}+ characters)`}><input type="password" value={secrets.adminPassword ?? ""} onChange={(event) => setSecrets({ ...secrets, adminPassword: event.target.value })} /></Field><Field label="Confirm password"><input type="password" value={passwordConfirmation} onChange={(event) => setPasswordConfirmation(event.target.value)} /></Field></div><div className="actions"><button onClick={() => void saveStructured(4)} disabled={!admin.email || !secrets.adminPassword || secrets.adminPassword !== passwordConfirmation}>Save and continue</button></div></Panel>}
...
function Installed(props: InstalledProps) {
  ...
  return <Panel title="Budgeted is installed" description="Later checks are optional. An interrupted mutation must be verified before another change.">
    ...
    <details><summary>Configuration and integration credentials</summary><div className="grid two top-gap"><Field label="Application name"><input value={props.configuration.appName} onChange={(event) => props.setConfiguration({ ...props.configuration, appName: event.target.value })} /></Field><Field label="Production stage"><input value={props.configuration.productionStage} onChange={(event) => props.setConfiguration({ ...props.configuration, productionStage: event.target.value })} /></Field></div><DomainFields configuration={props.configuration} setConfiguration={props.setConfiguration} /><IntegrationFields configuration={props.configuration} setConfiguration={props.setConfiguration} secrets={props.secrets} setSecrets={props.setSecrets} /><div className="top-gap"><Field label="Administrator password for seed retry"><input type="password" value={props.secrets.adminPassword ?? ""} onChange={(event) => props.setSecrets({ ...props.secrets, adminPassword: event.target.value })} /></Field></div><div className="actions"><button onClick={() => void props.saveStructured()}>Save configuration</button><button onClick={() => void props.start("prepare", { withSecrets: true })}>Prepare configuration and secrets</button><button disabled={!props.secrets.adminPassword} onClick={() => void props.start("seed", { withSecrets: true })}>Retry administrator seed</button></div></details>
